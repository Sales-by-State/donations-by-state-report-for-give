...
( function ( wp, config ) {
	...
	var API = '/dbsgive/v1/report';
	var DIAGNOSTICS = '/dbsgive/v1/diagnostics';
	var BACKFILL = '/dbsgive/v1/backfill';
	var DEFAULT_PER_PAGE = 25;
	var TEXTDOMAIN = 'donations-by-state-report-for-give';
	...
	function ImportStatus( props ) {
		...
		if ( ! diag.table_exists ) {
			return el(
				'div',
				{ className: 'dbsgive-notice is-error' },
				el( 'p', { className: 'dbsgive-notice__title' }, __( 'The report table is missing.', TEXTDOMAIN ) ),
		...
		return el(
			'div',
			{ className: 'dbsgive-importing', role: 'status', 'aria-live': 'polite' },
			el(
				'div',
				{ className: 'dbsgive-importing__row' },
				el( 'span', { className: 'dbsgive-importing__label' }, __( 'Reading past donations…', TEXTDOMAIN ) ),
				el( 'span', { className: 'dbsgive-importing__count' }, pct + '%' )
			),
			el(
				'div',
				{ className: 'dbsgive-progress' },
				el( 'div', { className: 'dbsgive-progress__bar', style: { width: pct + '%' } } )
			),
			el(
				'p',
				{ className: 'dbsgive-importing__hint' },
	...
	function readStatuses( query ) {
		var raw = query && query.dbsgiveStatuses;
	...
	function StatusFilter( props ) {
		...
		return el(
			'div',
			{ className: 'dbsgive-filter' },
			el( 'span', { className: 'dbsgive-filter__label' }, __( 'Donation status', TEXTDOMAIN ) ),
			el( Dropdown, {
				className: 'dbsgive-filter__dropdown',
				contentClassName: 'dbsgive-status-popover',
				...
				renderToggle: function ( toggleProps ) {
					return el(
						Button,
						{
							className: 'dbsgive-filter__toggle',
				...
				renderContent: function () {
					return el(
						'div',
						{ className: 'dbsgive-status-popover__list' },
	...
	function Filter( props ) {
		return el(
			'label',
			{ className: 'dbsgive-filter' },
			el( 'span', { className: 'dbsgive-filter__label' }, props.label ),
			el(
				'select',
				{
					className: 'dbsgive-filter__select',
					value: props.value,
					onChange: function ( event ) {
	 ...
	 * @return {Object} Element.
	 */
	function DonationsByStateReport( props ) {
		var query = props.query || {};
		...
		var country = query.dbsgiveCountry || settings.defaultCountry || 'US';
		var year = query.dbsgiveYear || settings.defaultYear;
		...
		var children = [
			...
			el(
				'div',
				{ key: 'filters', className: 'dbsgive-filters' },
				el( Filter, {
					label: __( 'Country', TEXTDOMAIN ),
					value: country,
					options: settings.countries || [],
					onChange: function ( value ) {
						setFilter( { dbsgiveCountry: value } );
					}
				} ),
				el( Filter, {
					...
					onChange: function ( value ) {
						setFilter( { dbsgiveYear: value } );
					}
				} ),
				el( StatusFilter, {
					value: statuses,
					onChange: function ( next ) {
						setFilter( { dbsgiveStatuses: next.join( ',' ) } );
					}
				} )
			)
		];
		...
		if ( data.error ) {
			children.push(
				el(
					'div',
					{ key: 'error', className: 'notice notice-error dbsgive-error' },
		...
		children.push(
			el( TableCard, {
				key: 'table',
				title: __( 'Donations by State', TEXTDOMAIN ),
				headers: headers,
				rows: rows,
				rowsPerPage: perPage,
		...
		return el( 'div', { className: 'woocommerce-analytics__report dbsgive-report' }, children );
	}
	...
	function StandaloneReport() {
		...
		return el( DonationsByStateReport, { query: query, setQuery: setQuery } );
	}
	...
	function readUrlQuery() {
		var query = {};
		var keys = [ 'dbsgiveCountry', 'dbsgiveYear', 'dbsgiveStatuses', 'orderby', 'order', 'paged', 'per_page' ];
		...
		query.page = found.page || 'dbsgive-donations-by-state';
	...
	function mountStandalone() {
		function render() {
			var node = document.getElementById( 'dbsgive-root' );

			if ( ! node ) {
				return;
			}

			if ( node.getAttribute( 'data-dbsgive-mounted' ) ) {
				return;
			}

			node.setAttribute( 'data-dbsgive-mounted', '1' );
	...
	mountStandalone();
} )( window.wp, window.dbsgiveConfig );
