...
/**
 * Turns a country, a year and a set of donation statuses into one row per state.
 */
class Report {
	 ...
	 * @param string|array $statuses Donation statuses to include.
	...
	/**
	 * Fetch donations grouped by billing state and status for one country and year.
	 *
	 * @param int    $year    Calendar year.
	 * @param string $country Two-letter country code.
	 * @return array
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT billing_state AS state,
				        status,
				        SUM( total_sales ) AS donations
				 FROM {$wpdb->prefix}dbsgive_order_state
	...
	private function total_by_state( array $rows, array $statuses ) {
		...
		foreach ( $rows as $row ) {
			...
			foreach ( Filters::measure_keys() as $key ) {
				$states[ $state ][ $key ] += (float) $row[ $key ];
			}
		}
	...
	/**
	 * Add a zero row for every state of the country that received nothing.
	 *
	 * @param array  $rows    One row per state with donations.
	 * @param string $country Two-letter country code.
	 * @return array
	 */
	...
	private function add_labels_and_currency( array $rows, $country ) {
		...
		foreach ( $rows as $index => $row ) {
			...
			$rows[ $index ]['state_name'] = isset( $state_names[ $code ] ) && $state_names[ $code ]
				? $state_names[ $code ]
				: ( '' === $code ? __( 'Unknown', 'donations-by-state-report-for-give' ) : $code );
	...
	/**
	 * A zero value for every measure column.
	 *
	 * @return array<string,float>
	 */
	private static function empty_measures() {
		$row = array();
	...
	private function money( $amount ) {
		...
		if ( function_exists( 'give_currency_filter' ) && function_exists( 'give_format_amount' ) ) {
			$formatted = give_currency_filter( give_format_amount( $amount ) );
