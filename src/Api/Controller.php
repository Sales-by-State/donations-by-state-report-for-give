...
namespace DBSGIVE\Api;

use DBSGIVE\Data\Backfill;
use DBSGIVE\Data\Report;
use DBSGIVE\Filters;
...
class Controller extends \WP_REST_Controller {
	 ...
	 * @var string
	 */
	protected $namespace = 'dbsgive/v1';
	...
	private function report_args() {
		return array(
			'year'     => array(
				'description' => __( 'Calendar year to report on.', 'donations-by-state-report-for-give' ),
				'type'        => 'integer',
				'default'     => Filters::default_year(),
			),
			'country'  => array(
				'description'       => __( 'Two-letter country code.', 'donations-by-state-report-for-give' ),
				'type'              => 'string',
				'default'           => Filters::default_country(),
				'sanitize_callback' => 'sanitize_text_field',
			),
			'statuses' => array(
				'description'       => __( 'Comma-separated donation statuses to include.', 'donations-by-state-report-for-give' ),
				'type'              => 'string',
	...
	public function get_report( $request ) {
		...
		if ( ! Schema::table_exists() ) {
			return new \WP_Error(
				'dbsgive_no_table',
				__( 'The report table could not be created. Check the database user has permission to create tables.', 'donations-by-state-report-for-give' ),
				array( 'status' => 500 )
			);
		}
	...
	/**
	 * Report on the state of the data rather than the donations.
	 *
	...
	public function get_item_schema() {
		...
		$row = array(
			'state'      => array(
				'description' => __( 'State code.', 'donations-by-state-report-for-give' ),
				'type'        => 'string',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
			...
			'state_name' => array(
				'description' => __( 'State name.', 'donations-by-state-report-for-give' ),
				'type'        => 'string',
				'context'     => array( 'view' ),
				'readonly'    => true,
			),
		...
		$this->schema = array(
			...
			'title'      => 'dbsgive_report',
			'type'       => 'object',
			'properties' => array(
				'rows'   => array(
					'description' => __( 'One entry per state.', 'donations-by-state-report-for-give' ),
					'type'        => 'array',
					'context'     => array( 'view' ),
					'readonly'    => true,
					'items'       => array(
				...
				'totals' => array(
					'description' => __( 'Totals across every state.', 'donations-by-state-report-for-give' ),
					'type'        => 'object',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'total'  => array(
					'description' => __( 'Number of states in the result.', 'donations-by-state-report-for-give' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
