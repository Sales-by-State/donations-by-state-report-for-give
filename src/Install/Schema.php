...
namespace DBSGIVE\Install;

use DBSGIVE\Data\OrderSource;
...
class Schema {
	...
	const DB_VERSION     = 1;
	const VERSION_OPTION = 'dbsgive_db_version';
	...
	public static function table() {
		...
		return $wpdb->prefix . 'dbsgive_order_state';
	}
	...
	public static function install() {
		...
		$sql = "CREATE TABLE {$wpdb->prefix}dbsgive_order_state (
		...
		if ( 0 === $counts['rows'] ) {
			delete_option( 'dbsgive_backfill_cursor' );
		}
	...
	public static function table_exists() {
		...
		return (bool) $wpdb->get_var(
			$wpdb->prepare( 'SHOW TABLES LIKE %s', $wpdb->prefix . 'dbsgive_order_state' )
		);
	}

	/**
	 * Row count alongside the number of donations on the site.
	 *
	...
	public static function counts() {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$rows = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->prefix}dbsgive_order_state" );
	...
	public static function truncate() {
		...
		$wpdb->query( "TRUNCATE TABLE {$wpdb->prefix}dbsgive_order_state" );
	}
}
